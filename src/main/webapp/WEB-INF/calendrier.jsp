...
<%-- <link href="style/${sessionScope.utilisateur.theme.nom.toLowerCase()}.css" rel="stylesheet" type="text/css"> --%>
<link href="style/${utilisateur.theme.nom.toLowerCase()}.css"
	rel="stylesheet">
</head>
<body>
	<h1>Calendrier</h1>
	<%-- <h2>Utilisateur : ${utilisateur.prenom} - solde : ${utilisateur.nbPoints} points <a href="deconnexion">Déconnexion</a></h2> --%>
	<h2>
		Utilisateur : ${utilisateur.prenom} - solde : ${utilisateur.nbPoints}
		points <a href="deconnexion">Déconnexion</a>
	</h2>
				...
				<c:choose>
					<c:when test="${jour.gif eq null }">
						<td colspan="3">${jour.nbPoints}points<br> <a
							href="placerGifDistant?ID_JOUR=${jour.date}">Placer un Gif
								distant</a>&nbsp; / &nbsp; <a
							href="placerGifTeleverser?ID_JOUR=${jour.date}">Placer un Gif
								Televerser</a><br>
						</td>
					</c:when>
...
${reaction.emotion.code} ${reaction.utilisateur.prenom}<br>
							</c:forEach> <a href="reagir?ID_GIF=${jour.gif.id}">Réagir</a></td>
					</c:when>
				</c:choose>
			</tr>
		</c:forEach>
